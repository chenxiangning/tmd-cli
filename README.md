...
**插件市场(插排)** —— 内置 27 位可视插拔(CLI 引擎 10 / 界面功能 14 / 核心系统 3 焊死),重启生效;本机插件(`~/.tmd-cli/plugins/`)免重启装载、对话即变自动重扫;在线市场入口预留
