...
- 内核零引擎知识:引擎声明(command/初始化参数/工作区探测)全部经 `registerLspEngine` 从插件注入;新增引擎 = 插件侧 discovery.ts 增一个 `discoverXxx` 发现函数 + `registerLspEngine` 注册一行,Rust/内核零改动。
...
- 引擎:TS(TS5→typescript-language-server / TS7→tsgo 自动分叉)、Python(pyright:venv→PATH→npx)、Java(jdt.ls 首用引导下载,里程碑钉 1.40.0);tsgo/pyright 的 npx 兜底不保证离线零安装。
