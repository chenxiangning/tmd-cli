...
4. **呼吸灯三态归内核 Host 结算（活动守望 1Hz）**：绿(2s 内有输出) → 蓝(静默结算时未被查看,
   ...
   空闲重绘闸(2026-09-11):CLI 空闲期状态栏/spinner 以光标寻址持续原地自绘
   (实测 omp ≈2.8KB/s),字节流上「有输出」与「对话进行中」不可区分 —— 判据 =
   剥 ANSI 后的可见骨架(仅 \p{L}\p{N})在会话最近 6 帧窗内恒定复现(实测空闲帧
   仅 4 种骨架;真实输出/计时帧每帧引入新字符)。复现帧不推活动钟、不开轮次,
   否则轮次永不静默结算,侧栏标签永挂「运行时」;ssh/shell 同豁免轮次开启闸。
   用户新提问清骨架窗(跨轮次全等输出不误判)。
