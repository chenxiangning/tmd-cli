...
4. **呼吸灯三态归内核 Host 结算（活动守望 1Hz）**：绿(2s 内有输出) → 蓝(静默结算时未被查看,
   ...
   (content/tick/static,2026-09-11 证据分级模型;2026-09-13 修订),静默 = content 停 >2s
   且轮次内跳过数字的活家具(ticker)帧流断供 >5s(持轮看帧流连续性,不看数字变动;
   omp 页脚过 60s 切分钟粒度);CLI 插件可声明 busyMarks(工作界面标记,omp 已声明),
   标记帧 = CLI 自证在途,刷 30s 自证钟持轮(2026-09-13 二修:流式期页脚与内容混片
   致骨架永远唯一、ticker 永不登记,>2s 流式间隙假结算且闸 4 拦死不自愈 —— 实采
   回放实证);未应答写入有 120s 天花板保护(思考期不假结算,到期必结算,busy 自证
...
- **events 是纯事件归因(2026-09-05 泄露回归定约,隔离优先于覆盖面)**:events 会话的
  ...
  证据,不破隔离(012210c 实证:13 个 edit 路径入账,15 个 bash 改写文件只进提交
