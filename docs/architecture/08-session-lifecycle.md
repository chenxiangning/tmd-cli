...
- **I1 对话锚定**:呼吸灯只认用户发起的对话。`host.writeSession` 的真实用户输入是唯一
  ...
  或尾 16KB 命中 busyMarks(现势:CLI 自证在途,兜底回显滚出 256KB 窗的长轮次)。
