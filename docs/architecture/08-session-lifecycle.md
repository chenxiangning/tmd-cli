...
|结算|调度间隙|系统睡眠/节流致墙钟跳变:三口钟同刻过窗,在途轮次被假结算且 I2 拦后续不自愈|结算 tick 间隔 >30s(`CLOCK_JUMP_MS` = `BUSY_HOLD_MS`,取最宽持轮窗:旧阈 60s 留 (30,60]s 睡眠带,自证钟刚好过窗即假结算;阈值随最宽窗联动,防日后调窗静默开带)= 观测缺口非因果证据:全在途钟刷到本刻跳过本轮;真静默后续 tick 照常结算|activityWatch.test.ts「调度间隙守卫」|
...
- 完工判定(已声明 busyMarks 的 CLI):换装(标记消失)后固定 +30s(`BUSY_HOLD_MS`)
  翻空闲/发结算 —— 结束音与未读标蓝比纯字节流判定的 5s 晚 25s,分钟级轮次无感;
  骨架推断路径(ticker)仍是 5s。**已同时声明 idleMarks 的 CLI(omp)例外(09-18)**:
  换装帧即武装,~2s 确认后收口,短轮次不再陪跑 30s 尾巴(§3 闸 4d)。
- resize 抑制窗内恰好完整到达的 <1s 短回答:漏提醒一次(需用户正在改尺寸同时成立)。
- **闸 4d 确认窗的理论误收口面(09-18)**:轮次在途且已应答后,若 CLI 在 >2s 帧静默间隙
  (深思期页脚稀疏形态)内出现「不带 busy 行的 mc 空闲帧」重绘,武装会在确认窗后提前结算,
  余下轮次被 I2 拦死。实采未见此形态(mc 行变更伴随 API/工具边界流量,同帧必有工作页脚,
  busy 优先即撤武装);omp 2026-09-18 全日志回放轮中零假结算。复发走 §7b。
