...
日期：2026-09-02
状态：设计已确认；评审修订 v4（UI 主形态 D，与 git 插件解耦，动作语义极简化，无筛选行）
...
- 纯时间线列表（无视图切换、**无筛选行**——v4 修订：状态徽标已挂在每个
  ...
  resume 断档线（断档前批次只读）。
- 两个点击目标、两个去处：**批次行点击 → 中央开「对话」tab**（该轮用户
  消息全文——数据源即锚点栏基础设施 `readSessionUserMessages` 的
  `CliUserMessage.text`，天然携带全文；下方附该轮 assistant 活动摘要；
  对话 tab 工具条带批级回退）；**文件行点击 → 中央开 diff tab**。
...
**中央编辑区 tab**（`editorCenter.tabContent` 挂载点，文件打开的容器位置）：

- **「对话」tab**：点批次行打开——该轮用户消息全文（转写卡样式，多行
  保留）+ 该轮 assistant 活动摘要；工具条 = 批次上下文 + 批级回退
  （仅 pending）。
- **diff tab**：点文件行打开——`#批次号 · 文件名`，tab 上带批次状态点；
  可开多个，可关闭；空态提示双入口。
- diff 工具条：路径 + ± + 批次上下文 +「回退此文件 / 回退整批」（仅
  pending 且未陈旧时出现）；陈旧文件显示"内容已变 —— 对照参考，不可回退"。
