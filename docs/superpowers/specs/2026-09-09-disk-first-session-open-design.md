...
- 日期:2026-09-09
- 状态:已落地 —— `9e49ffd`(主实现 + 拆自动激活)+ `515cd4d`/`55a7d87`(白屏时序修复:墓碑帧/连接遮罩保持到 CLI 首帧画完,清屏序列 `\x1b[2J` 后 300ms 一次切换),另含终审修复 `beb927a`。真机内存验收:冷启零预付 ≈0.5GB,22 分钟活跃态(实开 8 会话)整树 2.15GB,对照旧架构 3h 6.7GB→11.86GB 轨迹;WebContent 1.4GB → 330MB。
...
2. 辅线三件套(隐藏 tab 停解析/后台采样降频/滚回降配)**已废弃**(2026-09-09 二轮拍板):主线落地后 WebContent 实测 330MB,收益不抵 ①② 的行为代价与验证成本;若将来内存回潮,从实测数据重新立项,不沿旧账。
