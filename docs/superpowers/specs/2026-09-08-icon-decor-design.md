# 图标装饰设置:8 个界面图标的独立颜色与呼吸闪烁

日期:2026-09-08
状态:已确认(用户批准:自由取色 + 恢复默认;只换点亮色;范围 8 个)

## 背景与目标

一批界面图标的配色目前写死在 CSS(激活橙 `rgb(255,140,60)` 散落多处)或直接吃主题 token,用户无法按喜好调整,也没有统一的"呼吸闪烁"开关(新建会话火箭今日已手加一条写死呼吸)。

范围 = 用户指定的 8 个图标,每个图标两项设置(颜色 / 闪烁)互相独立:

| 键 | 图标 | 渲染点 | 现配色 |
|---|---|---|---|
| `eye` | 会话查看眼 | `.thread-viewing-eye`(3 处共用) | `var(--text-strong)` |
...
2. 自定义色即时生效、落盘持久;两态图标(面板 4 键 + `ssh-panel`/`system-proxy`)只替换点亮色,idle 保持主题灰;无两态(`eye`/`newchat`)恒色;
3. 闪烁 = 5s 超慢呼吸(opacity 1 → 0.35 → 1),每图标独立开关;`newchat` 出厂默认开(转正今日手加效果),其余默认关;`prefers-reduced-motion` 下全部关停。

## 方案取舍

**选定:通用设置 map + kernel 应用器 + CSS 变量约定。** `AppSettings.iconDecor: Record<键, { color?: string; blink?: boolean }>`,kernel 只懂"map → 往 `<html>` 写 `--icon-decor-<键>` 变量与 `data-icon-blink` 属性",不认识任何图标语义;各图标样式改吃 `var(--icon-decor-<键>, 原色)`;设置卡持静态 8 项清单(id/名称/预览组件)。新增可装饰图标 = CSS 一行 + 清单一行。
...
**否决:各渲染点内联读 settings**。eye 有 3 处渲染点,逐点订阅 + 内联 style 最散,live 切换最难维护。

## 数据与清洗

- `settingsTypes.ts`:`iconDecor: Record<IconDecorId, { color?: string; blink?: boolean }>`(`IconDecorId` = 上表 8 键联合);默认 `{ newchat: { blink: true } }`。
...
- 桩目检(1421 + Tauri IPC 桩):8 行渲染与折叠;改色 → 四类图标即时变色;闪烁开关 → 呼吸出现/消失;恢复默认 → 回写死橙/主题色;两态图标 idle 态不受影响。
