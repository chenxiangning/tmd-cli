...
3. 范围闸门:>240 改动文件需对话框内确认(带 fingerprint 防范围漂移),>300 标记 diff 不完整,README 命中且 ≥32 文件也需确认;
...
**选定:范围闸门原样复刻(阈值 240/300/32 + fingerprint)。** 防呆机制已被 mossx 验证,防误发超大 PR 与确认后范围漂移。

**上下文先例**:mossx 实现位于 `src-tauri/src/git/commands_pr_workflow.rs`(843 行单文件)+ `range_gate.rs` + 前端 `GitHistoryPanelView.tsx`;tmd-cli 按 300 行铁则拆 `pr_gh.rs` / `pr_gate.rs` / `pr_workflow.rs` 三文件,UI 拆 `CreatePrDialog.tsx` + `prDialogModel.ts`。

## 验证

- Rust:`parse_github_repo`、范围闸门判定(阈值/README 命中/fingerprint 一致性)单测;`cargo test && cargo clippy --all-targets -- -D warnings && cargo fmt --check`。
- 前端:`prDialogModel` 阶段状态机与 gate 确认流单测;`pnpm typecheck && pnpm test && pnpm check:arch-boundary && pnpm check:file-size && pnpm build`。
