...
- 封口可修订(有界):结算后、下一锚点前的继续改动以修订追加进同一批,读取以最后一行为准;变更集完全相同的重复封口不落冗余行;批发生回退/应用(审核态带守卫)即冻结 —— 修订重封不得按 live 重算,被回退删除的文件不再被剔出批(反悔清守卫即解冻,内容已回批后像时重封与原批等值不冗余)
...
- 回退冲突分层:live == 批后像整文件还原;M 文件(批前后像俱在)失配先走 diff 精准擦除(行级补丁,3 行上下文精确匹配,只擦本批 hunk,他人/手改写入保留);同区域改动重叠 = 「改动重叠」跳过,A/D 文件与前像缺失 = 「内容已变」跳过;绝无静默覆盖
