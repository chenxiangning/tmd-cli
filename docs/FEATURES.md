...
> 2026-09-05 审批线 review 加固:精准手术匹配改严格唯一(窗口内重复块歧义即冲突,不取最近启发式)、A 文件(批内新建)纳入擦除(空基线摘除本批写入块)、批冻结改回补式修订(live 重算 ∪ 回补封印文件,冻结期手改入修订不留审计洞)、apply 守卫用途澄清(冻结凭证,全量应用的撤销正路是再次回退)。
...
- 插排 / 清单双视图,18 个注册插件可视化插拔(8 engine + 7 feature + 3 core),写 settings.disabledPlugins,重启生效(运行期不热卸载)
