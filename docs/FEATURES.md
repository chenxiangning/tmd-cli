...
- 启动自动激活:按 天数窗口(默认 1 天,0 关)/ 每组配额(默认 1)/ 全局硬上限(默认 16)后台预开最近磁盘会话(不占 tab 不抢焦点),点击秒开命中身份去重直接聚焦;settings.autoActivateSessions,预开失败静默
...
- 会话 tab 右键菜单:重命名(行内输入,同侧栏契约,未落盘禁用)/ 关闭 / 关闭其他 / 关闭全部,与文件 tab 共用同一菜单组件
