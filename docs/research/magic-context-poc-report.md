...
日期:2026-09-05
状态:PoC-1 **通过**;PoC-2 **通过(d 路端到端闭环)**;PoC-5 **大半通过**;PoC-3 **omp 侧沉淀已验证**(胶囊命中留实施期);PoC-4 **隐含通过**(正式压测待实施期);**迁移已完成,环境就绪**
...
| **d · 借道 omp 会话代写(选定)** | tmd-cli 经 `proc_communicate` 跑 `omp -p "记住:…"`(非交互已实证可跑),omp 模型调 ctx_memory write,权威/epoch/FTS 全走官方管线;成本 = 一次小模型调用,秒级 |
...
- win 真机/VM 实测同 bootstrap(tasklist 枚举 + 路径 + node:sqlite on win);mac/linux 已实证。