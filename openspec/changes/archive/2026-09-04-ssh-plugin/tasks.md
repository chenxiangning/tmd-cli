...
- [x] Cargo.toml 加 russh 0.62.2 / russh-sftp 2.3;`cargo check` 通过
...
- [x] `ssh/commands.rs` + lib.rs 注册 + session.rs kind 字段 + write/resize/kill 路由
...
- [x] vitest 全绿 + typecheck + check:arch-boundary + check:file-size
...
- [x] `src/plugins/index.ts` 注册;`src/styles/ssh.css`
