...
> 本节所列 `budgetCommit.ts` / `BudgetPopover.tsx` 等文件最初落于 `src/plugins/workspace/`,
> 现已位于 `src/plugins/session-budget/`(独立插件,样式 `src/styles/session-budget.css`)。
> 下方路径已按代码现状修正(2026-09-06 评审)。

- **新增**:`src/plugins/session-budget/budgetCommit.ts`(提交校验纯函数)+ `budgetCommit.test.ts`
- **修改**:`src/plugins/workspace/SessionList.tsx`(初始 limit 接 quota)、`src/plugins/session-budget/BudgetPopover.tsx`(校验迁移至纯函数,UI 不变)
