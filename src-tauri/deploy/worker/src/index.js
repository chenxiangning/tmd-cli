/**
 * tmd-cli 中继（Cloudflare Worker）——让手机在外网访问本机 tmd-cli，而无需在
 * 本机开放任何入站端口。
 *
 ...
 * ---
 *
 * tmd-cli relay: lets a phone reach the desktop app without opening any
...
export class Relay {
  ...
  async fetch(request) {
    ...
    if (!this.agent || this.agent.readyState !== WebSocket.OPEN) {
      return new Response(
        "tmd-cli 桌面端未连接到中继。请在电脑上打开 tmd-cli → 设置 → 远程访问 → 外网访问，启动中转。",
