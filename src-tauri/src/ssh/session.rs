...
//!
//! 提示邮箱 → session_prompt.rs;自动重连 → session_reconnect.rs(文件规模铁则)。
...
use super::session_prompt::{ask_host_key, ask_user};
use super::transport::{connect_ssh_handle, CapturedHostKey, SshClient};
use super::{
    SshPromptEvent, SshRegistry, SshSessionEntry, SshSessionInput, SSH_DEFAULT_COLS,
    SSH_DEFAULT_ROWS, STATUS_CONNECTED, STATUS_CONNECTING, STATUS_FAILED,
};
...
pub(crate) async fn install_connected(
    app: &AppHandle,
    registry: &Arc<SshRegistry>,
    session_id: &str,
    entry: &Arc<SshSessionEntry>,
