...
/// 读版本库单文件(.versions/<file>)。内部原语(回退取内容用),无 IPC 面。
pub(crate) fn read_version_file(root: &Path, id: &str, file: &str) -> Result<String, String> {
    if !valid_name(file) {
...
pub fn archive_current(root: &Path, id: &str) -> Result<Option<String>, String> {
    ...
    if collect_stamps(root, &vdir)
        .iter()
        .any(|s| s.sha256 == sha256)
    {
        return Ok(None);
    }
    if links_symlink_under(root, &vdir) {
...
fn prune_versions(root: &Path, vdir: &Path) -> Result<(), String> {
    let mut stamps = collect_stamps(root, vdir);
...
pub(crate) fn version_of_file_name(file: &str) -> Option<&str> {
...
pub fn rollback(root: &Path, id: &str, file: &str) -> Result<(), String> {
    ...
    // 写路径与读路径同闸:入口文件与 plugin.json 都拒绝经软链写到 root 外
    if links_symlink_under(root, &dir.join(&entry))
        || links_symlink_under(root, &dir.join("plugin.json"))
    {
        return Err("拒绝写入符号链接".into());
    }
