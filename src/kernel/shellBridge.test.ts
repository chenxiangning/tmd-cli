 ...
 * - 非壳(无 window / 无桥):hasShellBridge=false,invoke reject「unavailable」
 * - iOS 壳:postMessage 帧 {id,method,args};__TMD_SHELL_RESULT__ 回注 resolve/reject
 * - Android 壳(2026-09-25):AndroidShell.post(JSON 串)同协议;log 同通道
...
/** 造 Android 壳 window:post 只收 JSON 串(addJavascriptInterface 原语限制)。 */
function fakeAndroidWindow() {
  const frames: Frame[] = [];
  const w = {
    AndroidShell: { post: (json: string) => frames.push(JSON.parse(json) as Frame) },
  };
  vi.stubGlobal("window", w);
  return { w, frames };
}
...
describe("shellBridge", () => {
  ...
  it("Android 通道:AndroidShell 检测 + JSON 串信封字段无损 + log 同通道", async () => {
    const { frames } = fakeAndroidWindow();
    bridge = await import("./shellBridge");
    expect(bridge.hasShellBridge()).toBe(true);
    const p = bridge.shellCreds.set('{"pin":"p"}');
    expect(frames).toHaveLength(1);
    expect(frames[0].method).toBe("creds.set");
    expect(frames[0].args).toEqual({ json: '{"pin":"p"}' });
    result(frames[0].id, true, null);
    await expect(p).resolves.toBeUndefined();
    bridge.shellLog("diag");
    expect(frames[1].id).toBe(0);
    expect(frames[1].method).toBe("log");
    expect(frames[1].args).toEqual({ line: "diag" });
  });
});
