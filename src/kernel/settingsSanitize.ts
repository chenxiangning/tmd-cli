...
import { sanitizeGitPanel } from "./settingsSanitizeGit";
...
/** 额度预警阈值清洗:0-100 整数钳制,非数字回落默认(0 = 关)。 */
function sanitizeClampPercent(raw: unknown): number {
  if (typeof raw !== "number" || !Number.isFinite(raw)) return DEFAULT_SETTINGS.notifyQuotaWarnPercent;
...
export function sanitize(raw: unknown): AppSettings {
  ...
  return {
    ...
    notifyOsAsk:
      typeof obj.notifyOsAsk === "boolean" ? obj.notifyOsAsk : DEFAULT_SETTINGS.notifyOsAsk,
    notifyOsTurnEnd:
      typeof obj.notifyOsTurnEnd === "boolean"
        ? obj.notifyOsTurnEnd
        : DEFAULT_SETTINGS.notifyOsTurnEnd,
    notifyOsSessionExit:
      typeof obj.notifyOsSessionExit === "boolean"
        ? obj.notifyOsSessionExit
        : DEFAULT_SETTINGS.notifyOsSessionExit,
    notifyQuotaWarnPercent: sanitizeClampPercent(obj.notifyQuotaWarnPercent),
