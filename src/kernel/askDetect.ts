...
/** ANSI 转义序列(OSC 整序列/CSI/单字符)——ansi-regex 同款成熟模式,只剥转义
 *  不伤可读文本。OSC 标题序列(ESC]0;…BEL/ST)必须整段剥离:CLI 私有的标题
 *  文本可含任意字符(汉字/空格/spinner glyph),旧窄字符类匹配失败只剥头部,
 *  标题残渣(~40 字符/帧)漏进可见文本 —— 污染 askDetect 标记窗口与
 *  activityWatch 的空闲重绘判定(2026-09-11 修复)。 */
const ANSI_RE =
...
/** 写后复燃抑制:仅「真作答」(写入时确有等待/候选/屏幕起算)上闸 —— 已答面板块
 *  会随整帧重绘在屏幕上逗留数秒(transcript 里同样含标记字面量),响应流将其推出
 *  屏幕后复现自然停止。普通发消息不上闸(其后是新提问不是残影)。预算约束:omp
 *  静态面板等待期 spinner ≈3.1KB/s,16KB 漂移 ≈5.3s 即撤销候选,抑制窗内的字节
 *  候选等不到窗过 —— 连续多问亮标由屏幕通道兜底(≈8s+防抖),勿把抑制窗当通用延迟。 */
