...
export function StatsCard({
  ...
  return (
    <div className={`mb-3 ${consoleCardCls}`}>
      <div className="mb-2 text-[0.71875rem] font-semibold">{t("统计")}</div>
      <div className="mb-2 flex flex-wrap gap-6">
        <div>
          <div className="font-mono text-xl font-bold">{counts.total}</div>
          <div className="text-[0.65625rem] text-(--tmd-fg-faint)">{t("记忆总数")}</div>
        </div>
        <div>
          <div className="font-mono text-xl font-bold">{counts.week}</div>
          <div className="text-[0.65625rem] text-(--tmd-fg-faint)">{t("本周新增")}</div>
        </div>
      </div>
      <div className="flex flex-col gap-1.5">
        {counts.byHarness.map(([name, n]) => (
          <div key={name} className="flex items-center gap-2">
            <span className="w-9 flex-none truncate text-[0.65625rem] text-(--tmd-fg-muted)">{name}</span>
            ...
            <span className="w-6 flex-none text-right font-mono text-[0.65625rem] text-(--tmd-fg-faint)">{n}</span>
          </div>
        ))}
      </div>
      {lastDream && <div className="mt-2 truncate text-[0.65625rem] text-(--tmd-fg-faint)">{t("上次整理:{time}", { time: lastDream })}</div>}
    </div>
  );
}
...
export function RecentCard({ recent }: { recent: MemoryItem[] }) {
  return (
    <div className={consoleCardCls}>
      <div className="mb-2 text-[0.71875rem] font-semibold">{t("最近沉淀")}</div>
      {recent.length === 0 ? (
        <div className="text-[0.6875rem] text-(--tmd-fg-faint)">{t("无记录")}</div>
      ) : (
        recent.map((m) => (
