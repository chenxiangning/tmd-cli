  ...
  return withDetails([
    {
      id: "roles",
      label: t("模型角色路由"),
      hint: t("modelRoles:供应商 → 模型:思考强度;候选 = omp 登录实况(omp models)∪ models.yml 已配置"),
      kind: "modelMap",
    ...
    },
    {
      id: "modelFallback",
      label: t("撞墙自动回退"),
      hint: t("retry.modelFallback(默认开)"),
      kind: "toggle",
    },
    {
      id: "chains",
      label: t("回退链"),
      hint: t("retry.fallbackChains:按角色配链,值 = 逗号分隔按序尝试;数组为整体替换语义"),
      kind: "modelMap",
      keyOptions: OMP_ROLES,
    },
    {
      ...
      label: t("全局思考强度"),
      hint: t("defaultThinkingLevel;角色后缀优先于此"),
      kind: "select",
    ...
    },
    {
      id: "prewalkEnabled",
      label: t("prewalk"),
      hint: t("prewalk.enabled:计划确定后自动切 smol 便宜模型跑实现"),
      kind: "toggle",
    },
    {
      id: "compactionEnabled",
      label: t("自动压缩历史"),
      hint: t("compaction.enabled"),
      kind: "toggle",
    },
    {
      id: "memoryBackend",
      label: t("记忆后端"),
      hint: t("memory.backend:mnemopi = 本地 SQLite 向量检索"),
      kind: "select",
    ...
    },
    {
      id: "webChain",
      label: t("网络搜索链"),
      hint: t("webSearch.providerChain:按序尝试"),
      kind: "orderedList",
    ...
    },
    {
      ...
      label: t("检查点"),
      hint: t("checkpoint.enabled:会话检查点(回滚锚点)"),
      kind: "toggle",
      advanced: true,
    },
    {
      id: "securityEnabled",
      label: t("安全扫描"),
      hint: t("security.enabled:提交前 secrets/恶意模式扫描"),
      kind: "toggle",
      advanced: true,
    },
    {
      id: "ttsrEnabled",
      label: t("流式工具摘要(ttsr)"),
      hint: t("ttsr.enabled:工具输出实时摘要(默认开)"),
      kind: "toggle",
      advanced: true,
    },
    {
      id: "ttsrInterrupt",
      label: t("ttsr 打断模式"),
      hint: t("ttsr.interruptMode:always = 随时可打断"),
      kind: "select",
