 ...
 * tab id = `ckpt-batch:<batchId>`;kind = "ckpt-batch";payload 携带 cwd + 深链文件。
 */
  ...
  openTab(
    {
      ...
      title: tab.title,
      path: tab.cwd,
      payload: { cwd: tab.cwd, batchId: tab.batchId, focusPath: tab.focusPath },
    },
    { refresh: true },
  );
}
...
export function readBatchPayload(tab: EditorTab): BatchTabPayload | null {
  ...
  if (!p?.cwd || !p.batchId) return null;
  return { cwd: p.cwd, batchId: p.batchId, focusPath: p.focusPath };
}
