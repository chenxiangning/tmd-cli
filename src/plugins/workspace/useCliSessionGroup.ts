 ...
 *
 * 职责:历史扫描(磁盘/远程分支见 useCliDiskScan.ts)、配额分页、
...
import { useCliDiskScan } from "./useCliDiskScan";
export function useCliSessionGroup({
  ...
  /* 活会话归属:本地 CLI 按 profileId;远程引擎会话(kind=ssh + engine,如 WSL 的
     omp 恢复)按 engine 归入引擎的 CLI 组 —— 否则绑定/标题/磁盘行去重全部失联,
     每点一次历史行就多一条无名主机名活行(2026-09-12 验收实证)。 */
  ...
  /* 历史扫描(磁盘/远程三分支 + 补扫)见 useCliDiskScan.ts。 */
  const { sessions, liveCliIds, setRescanTick } = useCliDiskScan({
    profile,
    workspace,
    liveSessions,
    refreshTick,
